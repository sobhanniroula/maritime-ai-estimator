...
export function parseAssessment(content: string): ParsedAssessment {
  ...
  for (const rawLine of content.split("\n")) {
    ...
    if (/^SITE CONDITIONS/.test(line)) { section = "conditions"; continue; }
    if (/^SITE SUITABILITY/.test(line)) { section = "suitability_block"; continue; }
    if (/^SUITABILITY:/.test(line)) {
      section = "suitability";
      out.suitabilityStatus = extractSuitability(line.replace(/^SUITABILITY:\s*/, ""));
      continue;
    }
    if (/^RECOMMENDED (SOLUTION|FOUNDATION|POOL|ANCHORING)/.test(line)) { section = "solution"; continue; }
    if (/^ANCHORING RECOMMENDATION/.test(line)) { section = "solution"; continue; }
    if (/^INDICATIVE PRICING/.test(line)) { section = "pricing"; continue; }
    ...
    if (/^NEXT STEPS/.test(line)) { section = "nextsteps"; continue; }
  ...
  return (
    <div className="space-y-3 text-xs w-full">
      ...
      <div className="pb-2 border-b border-slate-200 dark:border-slate-700">
        ...
        {d.location && (
          <div className="text-slate-700 dark:text-slate-200 font-mono text-[11px]">
            {d.location}
          </div>
        )}
        {d.date && (
      ...
      {d.suitabilityStatus && sc && (
        <div className={`rounded-lg px-3 py-2.5 border ${sc.border}`}>
          ...
          {d.suitabilityReason && (
            <div className="text-slate-600 dark:text-slate-300 text-[11px] leading-snug">
              {d.suitabilityReason}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1">
            {d.siteConditions.map(({ label, value }) => (
              <div
                key={label}
                className="bg-slate-100 dark:bg-slate-900/80 rounded-lg px-2.5 py-2"
              >
                <div className="text-[9px] text-slate-500 uppercase tracking-wide mb-0.5">
                  {label}
                </div>
                <div className="text-slate-700 dark:text-slate-200 text-[11px] leading-snug">
                  {value}
                </div>
              </div>
            ))}
      ...
      {d.solution.length > 0 && (
        <div>
          ...
          <div className="bg-slate-100 dark:bg-slate-900/80 rounded-lg divide-y divide-slate-200 dark:divide-slate-800">
          ...
          <div className="bg-slate-100 dark:bg-slate-900/80 rounded-lg divide-y divide-slate-200 dark:divide-slate-800">
            {d.pricing.map(({ label, value }) => (
              <div key={label} className="flex items-start gap-3 px-2.5 py-2">
                ...
                <span className="text-slate-700 dark:text-slate-200 text-[11px] leading-snug">
                  {value}
                </span>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="space-y-1">
            {d.permits.map((p, i) => (
              ...
              >
                <span className="text-slate-400 dark:text-slate-600 mt-0.5 shrink-0">☐</span>
